\section{\inGerman{Installieren von RetroShare}\inEnglish{Installation of RetroShare}}

\inGerman{Zunächst sei hier die offizielle Seite\footnote{offizielle Downloadseite: \url{http://retroshare.sourceforge.net/downloads.html}} von RetroShare empfohlen. Da RetroShare sich, trotz des Funktionsumfangs, in einem frühen Stadium befindet, ist die Entwicklung teilweise viel weiter als die stabilen Versionen, die auf der offiziellen Webseite zum herunterladen veröffentlicht werden. Deshalb sind meist zwei Versionen, eine stabile und ein Entwicklerschnappschuß, im Umlauf. Einsteiger, an die sich diese Anleitung richtet, sollten deshalb zur stabileren Versionen tendieren und nur jemand der sich auskennt sollte die Entwicklerversion benutzen.}
\inEnglish{First of all, have a look at the official download site\footnote{offizielle Downloadseite: \url{http://retroshare.sourceforge.net/downloads.html}}. But RetroShare is still in an active phase of development, so it can be that the downloadable versions on this website are very old and could contain already fixed bugs. }

\subsection{Windows}

\subsubsection{feste Installation}
\inGerman{Zur Intallation unter Windows lädt man von der offiziellen Seite die Version die auf *{\_}setup.exe endet herunter. Die zur Zeit (\today) aktuelle Version ist \href{http://sourceforge.net/projects/retroshare/files/RetroShare/0.5.3b/RetroShare_0.5.3b_4972_setup.exe/download}{RetroShare{\_}0.5.3b{\_}4972{\_}setup.exe}. Das für die Benutzung benötigte GnuPG ist bereits ins Installationspaket integriert. Während der Installation wird man gefragt ob man GnuPG installieren möchte. Hat man GnuPG bereits installiert, kann man diesen Schritt überspringen. Installiert man GnuPG kann man die Haken bei den Handbüchern entfernen, da RetroShare die Kommunikation mit GnuPG übernimmt.}
\inEnglish{\todo}

\subsubsection{\inGerman{RetroShare als portable Version}\inEnglish{portable Installation}}
\inGerman{Um es sich für den Anfang leicht zu halten und/oder um wenig Spuren auf dem Rechner zu hinterlassen, empfiehlt sich die Benutzung der auf der Downloadseite angebotenen \glqq Portablen Version\grqq. Sie ist während des Kompiliervorgang statisch gelinkt worden und so ist alles nötige in einem zip-Archiv enthalten. Nach dem Runterladen muß es nur noch an den Bestimmungsort entpackt werden. Ein Klick auf das im Ordner enthaltene \emph{RetroShare.exe} genügt und man kann gleich loslegen. Verknüpfungen in (Schnell-)/Startmenü oder auf Desktop müssen allerdings manuell gesetzt werden da sie von keinem Installationsprogramm gesetzt werden.}
\inEnglish{\todo}

\inGerman{In Kombination mit einer TrueCrypt- oder sonstig verschlüsselten Festplatte erhöht es die Sicherheit erheblich, da nach aushängen der \underline{verschlüsselten und versteckten} Partition es \textbf{glaubhaft abstreitbar} ist das RetroShare auf der Festplatte "'exisiert"'. Das höhere Maß an Sicherheit geht auf Kosten der Rechenleistung, da der PC zusätzlich zu GnuPG und OpenSSL, nun auch noch sich um die Verschlüsselung der Festplatte kümmern muß.}
\inEnglish{\todo}

\subsection{Linux}

\subsubsection{\inGerman{(K,Edu,L,X)Ubuntu und Ubuntu-Derivate\ldots}\inEnglish{(K,Edu,L,X)Ubuntu}}
...
\inGerman{Hierbei würde ich persönlich die Verwendung der \glqq retroshare-snapshots\grqq empfehlen, da diese häufiger aktualisiert werden und RetroShare noch ständig weiterentwickelt wird und man schneller an neue Entwicklungen kommt. Man beachte das die Installation beider Repositorities nichts nützt, weil der Paketname in beiden Verzeichnissen gleich ist und daher die höheren Versionsnummer bevorzugt wird!}
\inEnglish{Personally, I'm using the snapshots repository, as there are more updates and you get the new features faster.}
...
\inEnglish{Here things will get a little more complicated, but not much :)}

% Debian
\inGerman{\textbf{Debian} biedet weniger aktuelle Software als Ubuntu, legt dafür mehr Wert auf Stabilität. Deshalb sollte man das extra dafür angefertigte Paket\footnote{\url{http://sf.net/projects/retroshare/files/RetroShare/0.5.3b/RetroShare_0.5.3b.5129_debian_i386.deb}} herunterladen und installieren.}
...
\inGerman{Wenn für das eigene Betriebssystem kein Paket existiert, defekt ist oder man unbedingt das allerneueste von den Entwicklern haben möchte, bleibt als letzte Alternative nur das eigenständige kompilieren des Quelltextes.
...
\inEnglish{If no paket exists for your operating system, or you just want to have the very newest version, you can always compile RetroShare by yourself. It will require a little bit of programming knowledge, a guideline can be found at \href{http://retroshare.sourceforge.net/wiki/index.php/UnixCompile}{RetroShare-Wiki}.}
